...
public sealed class AesGcmFileEncryptor : IFileEncryptor
{
    ...
    private byte[] DeriveKey(FileEncryptionContext context, string purpose)
    {
        ...
        var info = Encoding.UTF8.GetBytes($"Zafiro.Sync:{purpose}:v1");
    ...
    {
        return Encoding.UTF8.GetBytes($"Zafiro.Sync:v1:{purpose}:{context.AppId}:{context.FileId}:{context.RevisionIntent}");
    }
