    ...
    CancellationToken cancellationToken) =>
{
    ...
    return Results.File(bytes, "application/json", $"appfilesync-{identity.AppId}-identity.json");
});
...
static AppFileSyncClient CreateSyncClient(DemoClientOptions options, AppIdentity identity, IHttpClientFactory httpClientFactory)
{
    var httpClient = httpClientFactory.CreateClient(nameof(AppFileSyncClient));
    httpClient.BaseAddress = options.ServiceBaseUri;
    var encryptor = new AesGcmFileEncryptor(identity.AppDataKey);

    return new AppFileSyncClient(
        httpClient,
...
public sealed class DemoClientOptions
{
    ...
    public string LocalIdentityPassword { get; init; } = "appfilesync-demo-local";
    ...
    public DemoClientOptions Validate()
    {
        ...
        _ = AppFileSync.Client.LogicalPath.Normalize(LogicalPath);
...
public sealed class DemoIdentityStore(DemoClientOptions options)
{
    ...
    {
        var identity = AppIdentity.Create(options.AppId, "AppFileSync Demo");
